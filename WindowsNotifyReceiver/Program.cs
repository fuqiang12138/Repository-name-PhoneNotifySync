...
internal sealed class MainForm : Form
{
    ...
    public MainForm()
    {
        ...
        Controls.Add(new Label { Text = "手机通知同步", Font = new Font("Microsoft YaHei UI", 22, FontStyle.Bold), AutoSize = true, Location = new Point(30, 25) });
        Controls.Add(new Label { Text = "● 电脑端正在监听 UDP 39555", Font = new Font("Microsoft YaHei UI", 12), AutoSize = true, Location = new Point(32, 78) });
        last = new Label { Text = "等待手机通知……", Font = new Font("Microsoft YaHei UI", 11), AutoSize = false, Size = new Size(450, 55), Location = new Point(32, 115) };
        Controls.Add(last);

        var test = new Button { Text = "测试电脑弹窗", Size = new Size(180, 42), Location = new Point(32, 190) };
        test.Click += (_, _) => ShowNotification("手机通知同步测试", "电脑端测试成功");
        Controls.Add(test);

        var firewall = new Button { Text = "放行 Windows 防火墙", Size = new Size(220, 42), Location = new Point(230, 190) };
        firewall.Click += (_, _) => FirewallHelper.AllowUdpPort();
        Controls.Add(firewall);

        Controls.Add(new Label
        {
            Text = "使用方法：\n1. 手机和电脑连接同一个 Wi‑Fi\n2. 点击一次“放行 Windows 防火墙”并在权限窗口点“是”\n3. 手机打开通知访问权限\n4. 点击手机 App 里的“测试电脑通知”",
            Font = new Font("Microsoft YaHei UI", 10), AutoSize = true, Location = new Point(32, 250)
        });
    ...
    {
        Show();
        if (activate) { WindowState = FormWindowState.Normal; Activate(); BringToFront(); }
    }
...
internal sealed class PopupForm : Form
{
    ...
    public PopupForm(string title, string body)
    {
        ...
        Controls.Add(new Label { Text = title, Font = new Font("Microsoft YaHei UI", 12, FontStyle.Bold), AutoSize = false, Size = new Size(320, 30), Location = new Point(18, 12) });
        Controls.Add(new Label { Text = body, Font = new Font("Microsoft YaHei UI", 10), AutoSize = false, Size = new Size(320, 45), Location = new Point(18, 48) });
...
internal sealed class UdpNotificationListener : IDisposable
{
    ...
    private readonly UdpClient udp = new(new IPEndPoint(IPAddress.Any, Port));
    private readonly CancellationTokenSource cts = new();
    public UdpNotificationListener(MainForm form) { this.form = form; udp.EnableBroadcast = true; }
    ...
    public void Dispose() { cts.Cancel(); udp.Dispose(); cts.Dispose(); }
}
...
internal static class FirewallHelper
{
    public static void AllowUdpPort()
    {
        try
        {
            ...
            {
                FileName = "netsh",
                Arguments = "advfirewall firewall add rule name=\"PhoneNotifySync UDP 39555\" dir=in action=allow protocol=UDP localport=39555 profile=any",
                UseShellExecute = true,
                Verb = "runas",
                WindowStyle = ProcessWindowStyle.Hidden
            };
            Process.Start(psi)?.WaitForExit();
            MessageBox.Show("已尝试放行 UDP 39555。现在回手机点击“测试电脑通知”。", "手机通知同步", MessageBoxButtons.OK, MessageBoxIcon.Information);
        }
        catch (Exception ex)
        {
            MessageBox.Show("没有获得管理员权限，请再次点击并在 Windows 权限窗口选择“是”。\n\n" + ex.Message, "手机通知同步", MessageBoxButtons.OK, MessageBoxIcon.Warning);
        }
    }
}
